...
- **主题预设**：`settings.theme`（graphite / warm / navy，`THEME_OPTIONS`），设置页「外观」分段切换，`lib/theme.ts` 把值写到 `<html data-theme>` 并缓存 localStorage 防闪，设置窗改完 `emit("friday://theme")` 广播给工作台即时换色。每个预设只是 `:root[data-theme=…]` 一组变量，布局不动；浅色主题未做。设置页 `.settings` 自身滚动（全局 html/body 是 overflow hidden）。
