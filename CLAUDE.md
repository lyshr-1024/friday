...
- **层次与深度（2026-09-11 下午）**：用户说精致化那轮改完「还是不行」，要我先去查别人怎么做。调研了 Linear 设计令牌、Vercel/Geist、Atlassian Elevation、ColorArchive 深色指南、Zylos/Mantlr 的 agent UX 模式等 11 个来源（笔记要点见下）。结论：**问题不在配色，在层次线索**——我们只有三级表面在用、一种边框到处用，眼睛判断不出谁包含谁。四条改动：
  ① **整圈 inset hairline ring**（Vercel/Geist 做法，最便宜见效最快）：`--ring-hairline: inset 0 0 0 1px rgba(255,255,255,.055)` + `--ring-top`（顶边更亮的高光 .07）合进 `--shadow-card`，再配 `0 1px 2px` 近影 + `0 8px 24px` 远影两层。原来只有顶边一条 .04 的高光，太弱。深色下阴影几乎看不见（Atlassian），厚度感主要来自这圈 ring。浅色主题的 ring 用深色半透明另给一套。
  ② **表面五级 + 边框四级**：按 ColorArchive 的 L\* 目标重算，相邻只差 3–5 点：`--bg-1` #0e1013（L\*4.6，离开纯黑，Linear 明令禁止纯黑做底）/ `--bg-2` #17191d（8.7）/ `--bg-3` #21252a（14.5，卡片）/ `--bg-4` #2b3037（19.7）/ `--bg-5` #363c44（25.1）。边框新增最暗一级 `--line-0`（#1e2126）专画卡片内部分隔线，`--line-1` 画外框，`--line-2` 留给 hover/focus——**分隔线必须比外框暗**，原来卡片外框用的是最重的 `--line-2`，内部分隔又和外框同级。四套主题各自补了 `--line-0`。
  ③ **左栏行高 56px → 45px**：调研指出 32–36px 适合纯扫描、44–56px 适合阅读决策，**40px 是两边都不讨好的折中**。标题改单行 ellipsis（选中那条才折两行看全）、标题行高 1.25、副信息 1.3、行内边距 6px。副信息（「需要你：回拂晓…」）是决策依据，没按调研建议藏到 hover。
  ④ **状态靠形态而非第二种颜色区分**（Linear 禁止第二强调色）：`.li--review/question/blocked` 用 `box-shadow: inset 2px 0 0` 画左侧竖条（等你审琥珀 / 卡住和在问红色，选中时加粗到 3px），不占布局；`.li__bar`（正在做）从行内 120px 小段改成贴行底整宽 2px 跑条——**「等你审」是静态有重量的，「正在做」是有时间维度的**（Zylos 的原则：明确区分 thinking 和 doing）。
  另外：19px 标题字距按 Linear 的表加到 -0.02em、13.5px 正文 -0.003em（Linear 的规律是负字距随字号加大，13–14px 归零，微标签反向用正字距）；11px 微标签旁的图标描边降到 1.25px（1.5px 在这个字号下比文字还重）；`.grp__head` 残留的 10px/8px 落标尺。
