 ...
 */
export type Intake =
  | { kind: "start"; project: string; detail: string; why: string }
...
export function intakePrompt(
  ...
  return {
    system: [
      ...
      "输出三选一：",
      '- {"kind":"start","project":"项目名","detail":"交给 Claude Code 的一段话","why":"一句话依据"}：问题说得够具体（改哪个页面、什么现象或什么要求），而且能对上注册表里的某个项目。detail 要把工单里的现象、复现步骤、页面链接原样带上，不要压缩成一句话。',
...
export function parseIntake(text: string, projects: Project[]): Intake {
  ...
  try {
    const r = JSON.parse(json) as { kind?: string; project?: string; detail?: string; question?: string; why?: string };
    ...
    if (r.kind === "start") {
      ...
      return { kind: "start", project: hit.name, detail: r.detail.trim().slice(0, 4000), why };
    }
