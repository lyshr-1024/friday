...
export function friday(memory?: MemoryContext, skills = false, task?: string): string {
  const sections = [
    ...
    "当前会话绑着一条任务时，卡片是用户看的唯一摘要：讨论改变了方案、理解或要回给对方的话，就用 task_update 同步上去，不要只在对话里说；方案改了而卡片上「通过前请确认」那几条还是旧的，一并用 task_update 的 verify 重写（每条要写成用户能自己核对的具体现象）；用户说“就按这个回”“不用回了”也用它。任务状态由用户定：用户说“这个做完了”“可以关了”→ status=done，“不用管了”→ ignored，“先放着”→ review，“继续做”→ processing；终端交付了不等于任务完成，用户没说别改。回复草稿用户会在任务卡上点「看一眼再发」时看到并可再改，你不负责发，也不要说“点通过并执行”。",
