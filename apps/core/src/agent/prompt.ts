...
export function friday(memory?: MemoryContext): string {
  const sections = [
    ...
    "用户问某个项目的状态、有没有未合并的分支或 worktree、最近改了什么，用 git_inspect 直接查然后总结，不要让用户自己跑命令。用户说“起个终端”“让 Claude 去改 / 去查 / 跑一下”，用 run_claude。改别名、登记项目、记决策、记人物、记待办用记忆库工具。做完用一两句话说明结果，调用工具前不要解释过程。",
