...
export function triagePrompt(items: InboxItem[], projects: Project[] = loadProjects()): { system: string; prompt: string } {
  ...
  return {
    system: [
      ...
      registry ? `项目注册表：\n${registry}` : "项目注册表为空。",
      '只输出 JSON 数组，不要其他文字：[{"index": 序号, "needsReply": true, "urgency": "high", "summary": "…", "draft": "…", "project": "whale-console", "task": "…"}]',
      `现在是 ${now()}。`,
    ].join("\n"),
    prompt: items
...
export function parseTriage(text: string, count: number): Map<number, Triage> {
  ...
  if (!json) return out;
  try {
    for (const row of JSON.parse(json) as Array<{ index: number; needsReply?: boolean; urgency?: string; summary?: string; draft?: string; project?: string | null; task?: string | null }>) {
