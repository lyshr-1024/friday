...
测试 **522 通过**（基线 493，新增 29），`pnpm typecheck` 干净，`cargo check` 退出 0、零 error。
...
5. **我在过程中误删过 `apps/core/src/api/summon.test.ts` 的原有内容**：要加 `pageHint` 用例时用 `cat >` 而不是追加，把原来那 5 个用例（`POST /summon` 两个、`POST /summon/act` 三个）整个覆盖掉了，`d7f2c0a` 那次提交里是缺的。已从 `261cdfb` 取回原文件、把 `pageHint` 三个用例追加在后面，补在最后一次提交里。现在 522 个用例，比基线多 29 个，没有净减少。下次写测试文件前先确认它存不存在。
